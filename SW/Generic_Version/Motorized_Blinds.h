...
class Motorized_Blinds : public Component, public CustomAPIDevice {
 ...
 public:
  void setup() override {
	...
	ESP_LOGD("custom", "Button setup complete.");
  ...
  void home_blinds(){
	...
	if(stepper_motor.getCurrentState() == BasicStepperDriver::STOPPED) {
		...
		ESP_LOGD("custom", "home_blinds ignored - already moving");
	}
